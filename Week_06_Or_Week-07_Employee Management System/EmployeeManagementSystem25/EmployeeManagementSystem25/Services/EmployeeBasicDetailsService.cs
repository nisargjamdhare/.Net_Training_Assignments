...
namespace EmployeeManagementSystem25.Services
{
    public class EmployeeBasicDetailsService : IEmployeeBasicDetailsService
    {
        ...
        public async Task<StudentDTO> AddStudentByMakePostRequest(StudentDTO studentDTO)
        {
            ...
            var responseObj = JsonConvert.DeserializeObject<StudentDTO>(requestObj);
            return responseObj;
        }

        public Task<StudentDTO> GetAllStudentsAsync(StudentDTO studentDTO)
        {
            throw new NotImplementedException();
        }

        public async Task<List<StudentDTO>> GetStudentByMakeGetReq()
        {
            var endPoint = "/api/Students/GetStudents";
            var response = await HttpClientHelper.MakeGetRequest(Credentials.StudentUrl, Credentials.GetAllStudentsEndPoint);
