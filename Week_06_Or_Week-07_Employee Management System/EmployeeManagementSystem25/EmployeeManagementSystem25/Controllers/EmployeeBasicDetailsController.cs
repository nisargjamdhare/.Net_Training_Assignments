...
namespace EmployeeManagementSystem25.Controllers
{
    ...
    public class EmployeeBasicDetailsController : ControllerBase
    {
        ...
        {
            var response = await _employeeBasicDetailsService.AddStudentByMakePostRequest(studentDTO);
            return Ok(response);
        }

        [HttpGet("GetStudentByMakeGetReq")]
        public async Task<IActionResult> GetStudentByMakeGetReq()
        {
            var students = await _employeeBasicDetailsService.GetStudentByMakeGetReq();
            return Ok(students);
        }
